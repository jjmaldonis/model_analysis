...
// Jason: For my 2D RDF images I usually use:  FitPosition(unblurred, blurred, 20)
...
// Given x0_rescale and y0_rescaled from FitPosition function,
// this outputs the distances from the center point to all the other
// points (sorted) into wave "seps"
function SeparationFromCenter(x,y)
	wave x,y
	
	//find point closest to the center
	variable i,j, centeri,dmin=100000
	for(i=0; i<DimSize(x,0); i+=1)
			if( sqrt(x(i)^2 + y(i)^2) < dmin)
				dmin = sqrt(x(i)^2 + y(i)^2)
				centeri = i
			endif
	endfor
	...
	for(i=0; i<DimSize(x,0); i+=1)
		if( (i != centeri) )
			InsertPoints 0,1, seps
			seps[0] = sqrt( (x(centeri)-x(i))^2 + (y(centeri)-y(i))^2)
		endif
	endfor
	Sort seps,seps
	DeletePoints 0,1, seps
	Display  root:seps
	ModifyGraph mode=2,lsize=2
	Label left "Inter-peak distances (A)"
	Label bottom "Arbitrary axis"
end
...
function FitPosition(unblurred, blurred, size)
	...
	GaussianFit(unblurred_n, x_loc, y_loc, size, 1)	// wiggle = 5
	
	duplicate/o x0 x0_rescaled
	duplicate/o y0 y0_rescaled
	
	duplicate/o x_loc x_loc_rescaled
	duplicate/o y_loc y_loc_rescaled
	
	//rescale x_0, y_0
	x0_rescaled = x0_rescaled*scalex+offsetx
	y0_rescaled = y0_rescaled*scaley+offsety
	
	x_loc_rescaled = x_loc_rescaled*scalex+offsetx
	y_loc_rescaled = y_loc_rescaled*scaley+offsety
	
	NewImage unblurred
	appendtograph/l/t y0_rescaled vs x0_rescaled
	ModifyGraph mode=3,marker=19,msize=1
	//appendtograph/l/t y_loc_rescaled vs x_loc_rescaled
	//ModifyGraph mode=3,marker=19,msize=1,rgb(y_loc_rescaled)=(0,15872,65280)
	
	SeparationFromCenter(x0_rescaled,y0_rescaled)
